...
= 3.1 19.10.2019 =
* Removed ability to serve inline scripts in banner - Security fix for CVE: "By exploiting the documented vulnerability, an authenticated attacker with high privileges (admin) can execute JavaScript code in a victim's browser."
* Some personal thoughts about the CVE:
* EU Cookie Law allows for free HTML editing of the banner via the admin area to give better customization. This can be done only by an user with **manage_options** (=**admin** rights).
